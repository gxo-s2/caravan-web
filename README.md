...
CaravanShare는 카라반(캠핑카) 소유자(호스트)와 이용을 원하는 사용자(게스트)를 연결하는 공유 플랫폼 서비스의 최소 기능 제품(MVP) 구현 프로젝트입니다. **바이브코딩 실전 문제** 시나리오를 바탕으로 **SOLID 원칙**을 적용하여 개발되었습니다.
...
| **2. 카라반 정보 관리** | • **카라반 등록/삭제:** 호스트가 카라반을 등록하고 삭제할 수 있는 기능.<br>• **정보 제공:** 수용 인원, 편의시설, 사진, 위치 등 상세 정보 포함.<br>• **검색/조회:** 게스트가 카라반을 검색/조회하고 상태(사용가능/예약됨/정비중)를 확인할 수 있음. |
